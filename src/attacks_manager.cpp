...
static void set_module_power(int mod_type) {
    ...
    bool nfc = (mod_type == 2);
    bool ble = (mod_type == 3);
    MorphDriver::powerOnModules(false, ble, nfc, cc);
}
...
static void task_ble_spam(void *pvParameters) {
    ...
    while (!stop_requested) {
        ...
        if (packet_count % 1000 == 0) {
            ...
            Serial.printf(
                "[BLE] %lu pacotes, %.0f pps, MAC: %02X:%02X:%02X:%02X:%02X:%02X\n",
                packet_count,
                pps,
                randomMac[0],
                randomMac[1],
...
static void task_wifi_deauth(void *pvParameters) {
    ...
    while (!stop_requested) {
        ...
        if (frame_count % 1000 == 0) {
            ...
            Serial.printf(
                "[DEAUTH] %lu frames, %.0f fps, alvo: %02X:%02X:%02X:%02X:%02X:%02X\n",
                frame_count,
                fps,
                t->bssid[0],
                t->bssid[1],
...
static void task_wifi_beacon_spam(void *pvParameters) {
    ...
    uint8_t beaconFrame[] = {
        ...
        0x00,
        0x00,
        0x00, // Source (random MAC)
        0x00,
        0x00,
        0x00,
        0x00,
        0x00,
        0x00, // BSSID (same as source)
        0x00,
        0x00, // Sequence
...
static void task_rf_spectrum(void *pvParameters) {
    ...
    while (!stop_requested) {
        ...
        // Give time for CPU but don't block scan speed significantly
        delay(1);
    }
...
void attacks_start(AttackType type) {
    ...
    switch (type) {
        ...
        case ATTACK_IR_TV_OFF:
            ...
            break;
        case ATTACK_IR_BRUTE: xTaskCreate(task_ir_brute, "Att_IR", 4096, NULL, 2, &attack_task_handle); break;
...
void attacks_report_fail() {
    ...
    ChameleonAnim::setMood(MOOD_ERROR);
}
