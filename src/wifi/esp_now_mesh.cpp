...
static void (*nfcRelayCallback)(const uint8_t *uid, uint8_t uidLen, const uint8_t *data, uint16_t dataLen) =
    nullptr;
    ...
    switch (msgType) {
        case MSG_TYPE_CHAT: {
            if (chatCallback && len >= sizeof(ChatPacket)) {
                ...
                char sender[20];
                sprintf(
                    sender, "%02X:%02X:%02X:%02X:%02X:%02X", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]
                );
...
void MeshSystem::addPeer(const uint8_t *mac) {
    ...
    if (esp_now_add_peer(&peerInfo) == ESP_OK) {
        Serial.printf(
            "[MESH] Peer adicionado: %02X:%02X:%02X:%02X:%02X:%02X\n",
            mac[0],
            mac[1],
            mac[2],
            mac[3],
...
void MeshSystem::sendChat(const uint8_t *mac, const char *message, bool broadcast) {
    ...
    strncpy(pkt.sender, "MorphNode", sizeof(pkt.sender));
...
// NFC RELAY
// ============================================================================
void MeshSystem::sendNFCRelay(
    const uint8_t *uid, uint8_t uidLen, const uint8_t *sectorData, uint16_t dataLen
) {
    if (!mesh_initialized) return;
...
void MeshSystem::onNFCRelay(
    void (*callback)(const uint8_t *uid, uint8_t uidLen, const uint8_t *data, uint16_t dataLen)
) {
...
bool MeshSystem::isRelayActive() { return relay_mode; }
...
// STATUS
// ============================================================================
bool MeshSystem::isInitialized() { return mesh_initialized; }

String MeshSystem::getMacAddress() { return WiFi.macAddress(); }
