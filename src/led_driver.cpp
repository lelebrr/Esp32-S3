...
void LEDDriver::setAll(uint32_t color) {
    for (int i = 0; i < LED_COUNT; i++) { _pixels[i] = color; }
}
...
void LEDDriver::setBrightness(uint8_t brightness) { _brightness = brightness; }
...
void LEDDriver::matrixRain() {
    ...
    if (!init) {
        for (int i = 0; i < LED_COUNT; i++) { speeds[i] = random(5, 20); }
        init = true;
    }
