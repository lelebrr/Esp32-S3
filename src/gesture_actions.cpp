...
void GestureActions::executeGestureAction(GestureType gesture) {
    ...
    const char *gestureNames[] = {
        "NONE", "UP", "DOWN", "LEFT", "RIGHT", "FORWARD", "BACKWARD", "CLOCKWISE", "COUNTERCLOCKWISE", "WAVE"
    };
...
bool GestureActions::isEnabled() { return _enabled; }

void GestureActions::setMapping(GestureType gesture, AttackType attack) {
    if (gesture >= 0 && gesture < 10) { _mappings[gesture] = attack; }
}

AttackType GestureActions::getMappedAttack(GestureType gesture) {
    if (gesture >= 0 && gesture < 10) { return _mappings[gesture]; }
    return ATTACK_NONE;
}
...
bool GestureActions::saveMappings(const char *filename) {
    ...
    for (int i = 0; i < 10; i++) { doc[gestureNames[i]] = (int)_mappings[i]; }
