...
static void tts_playback_task(void *param) {
    ...
    while (bytes_read < header.data_size && tts_playing) {
        size_t to_read = min((size_t)(header.data_size - bytes_read), sizeof(buffer));
        size_t actual = wav.read((uint8_t *)buffer, to_read);

        if (actual == 0) break;

        // Apply volume
        for (int i = 0; i < actual / 2; i++) { buffer[i] = (int16_t)(buffer[i] * tts_volume); }
...
void think_and_speak(const char *text) {
    ...
    } else if (strstr(text, "bateria") != nullptr || strstr(text, "battery") != nullptr) {
        tts_speak("bateria_baixa"); // Generic for now
    } else if (strstr(text, "sistema_pronto") != nullptr) {
        tts_speak("sistema_pronto");
    }
}

void tts_notify_battery(int level) {
    if (level < 20) tts_speak("bateria_baixa");
}

void tts_notify_gps() {
    // We don't have a specific "GPS Locked" wave, reuse system ready or just log
    // tts_speak("sistema_pronto");
}
...
bool tts_is_playing() { return tts_playing; }
