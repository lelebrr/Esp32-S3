/**
 * @file logger.cpp
 * @brief Logging System for MorphNode
 *
...
static String get_timestamp() {
    ...
    if (days > 0) {
        snprintf(buf, sizeof(buf), "D%lu %02lu:%02lu:%02lu", days, hr % 24, min % 60, sec % 60);
    } else {
        snprintf(buf, sizeof(buf), "%02lu:%02lu:%02lu.%03lu", hr % 24, min % 60, sec % 60, ms % 1000);
    }
    return String(buf);
}
...
void log_attack(const char *attack_name, bool success) {
    ...
    char buf[128];
    snprintf(buf, sizeof(buf), "ATTACK %s: %s", attack_name, success ? "SUCCESS" : "FAIL");
