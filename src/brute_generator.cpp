...
static bool _initBuffer() {
    ...
    if (!_writeBuffer) { _writeBuffer = (char *)malloc(WRITE_BUFFER_SIZE / 4); }
    _bufferPos = 0;
...
static void _writeLine(const char *line) {
    ...
    if (_bufferPos + len + 1 >= bufSize) { _flushBuffer(); }
...
void brute_set_progress_callback(BruteProgressCallback callback) { _progressCallback = callback; }
...
uint32_t brute_generate_porteiros() {
    ...
    for (int i = 0; i <= 9999 && !_cancelled; i++) {
        ...
        if (i % 1000 == 0 && _progressCallback) {
            if (!_progressCallback(count, 10000, "Porteiros")) { _cancelled = true; }
        }
    }
...
uint32_t brute_generate_datas() {
    ...
    for (int year = 1950; year <= 2025 && !_cancelled; year++) {
        ...
        if (year % 10 == 0 && _progressCallback) {
            if (!_progressCallback(count, 200000, "Datas")) { _cancelled = true; }
        }
    }
...
uint32_t brute_generate_wifi_br() {
    ...
    for (int p = 0; p < numPrefixes && !_cancelled; p++) {
        ...
        if (_progressCallback) {
            if (!_progressCallback(count, numPrefixes * 11000, "WiFi BR")) { _cancelled = true; }
        }
    }
...
uint32_t brute_generate_ceps() {
    ...
    if (_progressCallback) { _progressCallback(count, 25000, "CEPs"); }
...
uint32_t brute_generate_nomes_br() {
    ...
    const char *nomes[] = {"maria",   "jose",     "ana",     "joao",     "antonio",  "francisco",
                           "carlos",  "paulo",    "pedro",   "lucas",    "gabriel",  "rafael",
                           "daniel",  "marcelo",  "bruno",   "felipe",   "rodrigo",  "gustavo",
                           "andre",   "fernando", "julia",   "amanda",   "beatriz",  "camila",
                           "larissa", "leticia",  "mariana", "patricia", "fernanda", "bruna"};
    ...
    if (_progressCallback) { _progressCallback(count, 2000, "Nomes BR"); }
...
uint32_t brute_generate_wps_pins() {
    ...
    for (int i = 0; i <= 9999 && !_cancelled; i++) {
        ...
        if (i % 2000 == 0 && _progressCallback) {
            if (!_progressCallback(count, 10000, "WPS PINs")) { _cancelled = true; }
        }
    }
