...
bool PMKIDCapture::extractPMKID(const uint8_t *data, size_t len, uint8_t *pmkid) {
    ...
    while (offset + 4 < len) {
        ...
        if (kdeLen >= 20) {
            if (data[offset + 2] == 0x00 && data[offset + 3] == 0x0F && data[offset + 4] == 0xAC &&
...
void PMKIDCapture::start(const uint8_t *targetBSSID) {
    ...
    if (targetBSSID) {
        ...
        _hasTarget = true;
        Serial.printf(
            "[PMKID] Target: %02X:%02X:%02X:%02X:%02X:%02X\n",
            _targetBSSID[0],
            _targetBSSID[1],
            _targetBSSID[2],
            _targetBSSID[3],
...
bool PMKIDCapture::isCapturing() { return _capturing; }

int PMKIDCapture::getCaptureCount() { return _captureCount; }

PMKIDResult PMKIDCapture::getCapture(int index) {
    if (index >= 0 && index < _captureCount) { return _captures[index]; }
