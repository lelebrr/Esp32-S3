...
String getScriptsFolder(FS *&fs) {
    String folder;
    String possibleFolders[] = {"/scripts", "/LeleScripts", "/LeleJS"};
...
void ScriptsMenu::drawIconImg() {
    drawImg(
        *leleConfig.themeFS(),
        leleConfig.getThemeItemImg(leleConfig.theme.paths.interpreter),
        0,
        imgCenterY,
        true
    );
...
void ScriptsMenu::drawIcon(float scale) {
    ...
    // File
    tft.drawRect(iconCenterX - iconW / 2, iconCenterY - iconH / 2, iconW, iconH, leleConfig.priColor);
    tft.fillRect(
        iconCenterX + iconW / 2 - foldSize, iconCenterY - iconH / 2, foldSize, foldSize, leleConfig.bgColor
    );
    tft.drawTriangle(
