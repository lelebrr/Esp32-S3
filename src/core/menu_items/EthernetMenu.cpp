...
void EthernetMenu::drawIconImg() {
    drawImg(
        *leleConfig.themeFS(),
        leleConfig.getThemeItemImg(leleConfig.theme.paths.ethernet),
        0,
        imgCenterY,
        true
    );
...
void EthernetMenu::drawIcon(float scale) {
    ...
    |     |
    */
    tft.drawRect(starterX, Y, lineWidth, iconH, leleConfig.priColor);

    tft.drawRect(finalX, Y, lineWidth, iconH, leleConfig.priColor);

    tft.drawRect(starterX, Y, iconW * 2, lineWidth, leleConfig.priColor);
    ...
    |-   -|
    */
    tft.drawRect(starterX, Y + iconH, smallerH, lineWidth, leleConfig.priColor);

    tft.drawRect(finalX - smallerH + lineWidth, Y + iconH, smallerH, lineWidth, leleConfig.priColor);
    ...
    |-    -|
      |  |
    */
    tft.drawRect(starterX + smallerH, Y + iconH, lineWidth, smallerH, leleConfig.priColor);
    tft.drawRect(finalX - smallerH + lineWidth, Y + iconH, lineWidth, smallerH, leleConfig.priColor);

    // Draw the four cable pin at a distance of 15 pixel
    for (size_t i = 0; i < 4; i++) {
        tft.drawRect(starterX + 15 + (i * 15), Y, lineWidth, 16, leleConfig.priColor);
    }
