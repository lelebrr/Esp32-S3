...
void LeleTheme::removeTheme(void) {
    themeInfo t;
    theme = t;
}
FS *LeleTheme::themeFS(void) {
    if (theme.fs == 1) return &LittleFS;
    else if (theme.fs == 2) return &SD;
    return &LittleFS; // always get back to safety
}
bool LeleTheme::openThemeFile(FS *fs, String filepath, bool overwriteConfigSettings) {
    ...
    ThemeEntry entries[] = {
        ...
        {"Arquivos",       &theme.files,       theme.paths.files      },
        ...
        {"Outros",      &theme.others,      theme.paths.others     },
        {"usb",         &theme.usb,         theme.paths.usb        },
        {"Conectar",     &theme.connect,     theme.paths.connect    },
    ...
    if (overwriteConfigSettings) {
        uint16_t _priColor = leleConfig.priColor;
        uint16_t _secColor = leleConfig.secColor;
        uint16_t _bgColor = leleConfig.bgColor;
...
#ifdef HAS_RGB_LED
        if (!_th["ledBright"].isNull()) { leleConfig.ledBright = _th["ledBright"].as<int>(); }
        if (!_th["ledColor"].isNull()) { leleConfig.ledColor = strtoul(_th["ledColor"], nullptr, 16); }
        if (!_th["ledEffect"].isNull()) { leleConfig.ledEffect = _th["ledEffect"].as<int>(); }
        if (!_th["ledEffectSpeed"].isNull()) { leleConfig.ledEffectSpeed = _th["ledEffectSpeed"].as<int>(); }
        if (!_th["ledEffectDirection"].isNull()) {
            leleConfig.ledEffectDirection = _th["ledEffectDirection"].as<int>();
        }
        ledSetup();
#endif
    }
...
bool LeleTheme::validateImgFile(FS *fs, String filepath) {
...
void LeleTheme::_setUiColor(uint16_t primary, uint16_t *secondary, uint16_t *background) {
    priColor = primary;
