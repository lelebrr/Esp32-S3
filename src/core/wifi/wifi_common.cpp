...
bool _wifiConnect(const String &ssid, int encryption) {
    String password = leleConfig.getWifiPassword(ssid);
    ...
    if (connected) {
        ...
        leleConfig.addWifiCredential(ssid, password);
        updateClockTimezone();
    }
...
bool _setupAP() {
    ...
    WiFi.softAP(leleConfig.wifiAp.ssid, leleConfig.wifiAp.pwd, 6, 0, 4, false);
    wifiIP = WiFi.softAPIP().toString(); // update global var
    LOG_WIFI("IP: %s", wifiIP.c_str());
    wifiConnected = true;
    return true;
}
...
bool wifiConnectMenu(wifi_mode_t mode) {
    ...
    switch (mode) {
        ...
        default: // error handling
            LOG_WARN("Unknown wifi mode: %d", mode);
            break;
    }
...
void wifiConnectTask(void *pvParameters) {
    ...
    for (int i = 0; i < nets; i++) {
        ssid = WiFi.SSID(i);
        pwd = leleConfig.getWifiPassword(ssid);
        if (pwd == "") continue;
...
bool wifiConnecttoKnownNet(void) {
    ...
    for (int i = 0; i < nets; i++) {
        ...
        if (result) {
            LOG_WIFI("Connected to: %s", ssid.c_str());
            break;
        }
    }
    if (WiFi.status() == WL_CONNECTED) {
