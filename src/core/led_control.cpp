...
void ledEffectTask(void *pvParameters) {
    ...
    while (1) {
        CRGB baseColor = isPreviewLed ? previewLedColor : leleConfig.ledColor;
        int ledEffect = isPreviewLed ? previewLedEffect : leleConfig.ledEffect;
        int ledEffectSpeed = isPreviewLed ? previewLedEffectSpeed : leleConfig.ledEffectSpeed;
        int ledEffectDirection = isPreviewLed ? previewLedEffectDirection : leleConfig.ledEffectDirection;
...
void beginLed() {
    ...
    */
    ...
    setLedBrightness(leleConfig.ledBright);
}

void blinkLed(int blinkTime) {
    if (!leleConfig.ledBlinkEnabled) return;

    int ledBrightFrom = leleConfig.ledBright;
...
#define LelePurple 9830500 // Custom purple color for Lele
...
void setLedColorConfig() {
    ...
    while (1) {
        ...
        for (const auto &mapping : colorMappings) {
            if (leleConfig.ledColor == mapping.color) { idx = i; }
            colorStorage[i] = mapping.color;

            options.emplace_back(
                mapping.name,
                [=, &mapping]() {
                    // leleConfig.setLedColor(mapping.color);
                    leleConfig.setLedColor(
        ...
        options.push_back(
            {"Custom Color",
             ...
             [](void *pointer, bool shouldRender) {
                 setLedColor(leleConfig.ledColor);
                 return false;
             }}
        );
...
void setCustomColorSettingMenu(int rgb, std::function<uint32_t(uint32_t, int)> colorGenerator) {
    uint32_t originalColor = leleConfig.ledColor;
    ...
    while (i <= 255) {
        if (i % LED_COLOR_STEP == 0 || i == 255) {
            ...
            options.emplace_back(
                String(i),
                [updatedColor]() { leleConfig.setLedColor(updatedColor); },
                selectedIndex == index,
                hoverFunction,
                &colorStorage[index]
            );
...
void setLedEffectConfig() {
    ...
    while (1) {
        options = {
            {"Solid Color",
             [=]() { leleConfig.setLedEffect(LED_EFFECT_SOLID); },
             leleConfig.ledEffect == LED_EFFECT_SOLID,
             [](void *pointer,                                                                     bool shouldRender) {
            ...
            {"Breathe",
             [=]() { leleConfig.setLedEffect(LED_COLOR_BREATHE); },
             leleConfig.ledEffect == LED_COLOR_BREATHE,
             [](void *pointer,                                                                     bool shouldRender) {
                 setLedEffect(LED_COLOR_BREATHE);
                 return false;
             }                                                                    },
            {"Color Cycle",
             [=]() { leleConfig.setLedEffect(LED_EFFECT_COLOR_CYCLE); },
             leleConfig.ledEffect == LED_EFFECT_COLOR_CYCLE,
             [](void *pointer,                                                                     bool shouldRender) {
...
#if LED_COUNT > 1
            {"Color Wheel",
             [=]() { leleConfig.setLedEffect(LED_EFFECT_COLOR_WHEEL); },
             leleConfig.ledEffect == LED_EFFECT_COLOR_WHEEL,
             [](void *pointer,                                                                     bool shouldRender) {
                 setLedEffect(LED_EFFECT_COLOR_WHEEL);
                 return false;
             }                                                                    },
            {"Chase",
             [=]() { leleConfig.setLedEffect(LED_EFFECT_CHASE); },
             leleConfig.ledEffect == LED_EFFECT_CHASE,
             [](void *pointer,                                                                     bool shouldRender) {
                 setLedEffect(LED_EFFECT_CHASE);
                 return false;
             }                                                                    },
            {"Chase Tail",
             [=]() { leleConfig.setLedEffect(LED_EFFECT_CHASE_TAIL); },
             leleConfig.ledEffect == LED_EFFECT_CHASE_TAIL,
             [](void *pointer,                                                                     bool shouldRender) {
            ...
            {"Config - Speed",
             ...
             [](void *pointer,                                                                     bool shouldRender) {
                 previewLedEffect = leleConfig.ledEffect;
                 previewLedEffectSpeed = leleConfig.ledEffectSpeed;
                 previewLedEffectDirection = leleConfig.ledEffectDirection;
                 return false;
             }                                                                    },
        ...
        int selectedOption = loopOptions(options, leleConfig.ledEffect);
        if (selectedOption == -1 || selectedOption == options.size() - 1) {
            ledPreviewMode(false);
            ledSetup();
            return;
...
void setLedEffectSpeedConfig() {
    ...
    while (i < 10) {
        ...
        bool isSelected = (leleConfig.ledEffectSpeed == i + 1);

        options.emplace_back(
            label,
            [i]() { leleConfig.setLedEffectSpeed(i + 1); },
            (leleConfig.ledEffectSpeed == i + 1),
            hoverFunction,
            &speedStorage[i]
        );
        ++i;
    ...
    options.emplace_back(
        "Sync To Encoder",
        []() { leleConfig.setLedEffectSpeed(11); },
        (leleConfig.ledEffectSpeed == 11),
        hoverFunction,
        &speedStorage[10]
    );
#endif
    ...
    int selectedOption = loopOptions(options, leleConfig.ledEffectSpeed - 1);
    if (selectedOption == -1 || selectedOption == options.size() - 1) {
        previewLedEffectSpeed = leleConfig.ledEffectSpeed;
        return;
    }
...
void setLedEffectDirectionConfig() {
    options = {
        {"Clockwise",
         [=]() { leleConfig.setLedEffectDirection(1); },
    ...
    int selectedOption = loopOptions(options, (leleConfig.ledEffectDirection == 1) ? 0 : 1);
    if (selectedOption == -1 || selectedOption == options.size() - 1) {
        previewLedEffectDirection = leleConfig.ledEffectDirection;
        return;
    }
...
void ledSetup() {
    if (leleConfig.ledEffect == LED_EFFECT_SOLID) { ledEffects(false); }

    if (leleConfig.ledEffect > LED_EFFECT_SOLID) {
        ledEffects(true);
    } else setLedColor(leleConfig.ledColor);
}
...
void ledPreviewMode(bool enable) {
    isPreviewLed = enable;
    if (enable) {
        previewLedColor = leleConfig.ledColor;
        previewLedEffect = leleConfig.ledEffect;
        previewLedEffectSpeed = leleConfig.ledEffectSpeed;
        previewLedEffectDirection = leleConfig.ledEffectDirection;
    }
...
void setLedBrightnessConfig() {
    int idx = 0;
    if (leleConfig.ledBright == 0) idx = 0;
    else if (leleConfig.ledBright == 10) idx = 1;
    else if (leleConfig.ledBright == 25) idx = 2;
    else if (leleConfig.ledBright == 50) idx = 3;
    else if (leleConfig.ledBright == 75) idx = 4;
    else if (leleConfig.ledBright == 100) idx = 5;

    options = {
        {"OFF",
         [=]() { leleConfig.setLedBright(0); },
         leleConfig.ledBright == 0,
         [](void *pointer, bool shouldRender) {
             setLedBrightness(0);
             return false;
         }},
        {"10 %",
         [=]() { leleConfig.setLedBright(10); },
         leleConfig.ledBright == 10,
         [](void *pointer, bool shouldRender) {
             setLedBrightness(10);
             return false;
         }},
        {"25 %",
         [=]() { leleConfig.setLedBright(25); },
         leleConfig.ledBright == 25,
         [](void *pointer, bool shouldRender) {
             setLedBrightness(25);
             return false;
         }},
        {"50 %",
         [=]() { leleConfig.setLedBright(50); },
        ...
        {"75 %",
         [=]() { leleConfig.setLedBright(75); },
         leleConfig.ledBright == 75,
         [](void *pointer, bool shouldRender) {
             setLedBrightness(75);
             return false;
         }},
        {"100%",
         [=]() { leleConfig.setLedBright(100); },
         leleConfig.ledBright == 100,
         [](void *pointer, bool shouldRender) {
