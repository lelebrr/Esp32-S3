...
void displayScrollingText(const String &text, Opt_Coord &coord) {
    ...
    if (len < coord.size) {
        ...
        tft.fillRect(
            coord.x, coord.y, (coord.size - 1) * LW * tft.textsize, LH * tft.textsize, leleConfig.bgColor
        ); // Clear display area
...
void TouchFooter(uint16_t color) {
    ...
    tft.drawCentreString("OK", tftWidth / 2, tftHeight + 4, 1);
...
void displayInfo(String txt, bool waitKeyPress) {
    // Quebra automática de linhas para textos longos
    int maxCharsPerLine = (tftWidth - 40) / (LW * FP);
    if (txt.length() > maxCharsPerLine * 2) {
        // Texto muito longo - divide em até 3 linhas
        String line1 = txt.substring(0, maxCharsPerLine);
        String line2 = txt.substring(maxCharsPerLine, maxCharsPerLine * 2);
        String line3 = txt.substring(maxCharsPerLine * 2);
...
void displayTextLine(String txt, bool waitKeyPress) {
    ...
    displayRedStripe(txt, getComplementaryColor2(leleConfig.priColor), leleConfig.priColor);
#ifndef HAS_SCREEN
...
int loopOptions(
    ...
    while (1) {
        ...
        if (menuType == MENU_TYPE_MAIN) {
            checkReboot();
            if (devModeCounter >= 5 && !leleConfig.devMode) {
                leleConfig.setDevMode(true);
        ...
        if (PrevPress || check(UpPress)) {
            ...
            tft.drawArc(
                tftWidth / 2, tftHeight / 2, 25, 15, 0, 360, leleConfig.bgColor, leleConfig.bgColor
            );
            LongPress = false;
#endif
        ...
        if (check(NextPress) || check(DownPress)) {
            ...
            if ((index + 1) > options.size()) {
                if (!leleConfig.devMode) devModeCounter++;
                index = 0;
            }
            redraw = true;
        }
...
void progressHandler(int progress, size_t total, String message) {
    ...
    if (barWidth < 3) {
        tft.fillRect(6, 27, tftWidth - 12, tftHeight - 33, leleConfig.bgColor);
        tft.drawRect(18, tftHeight - 47, tftWidth - 36, 17, leleConfig.priColor);
        displayRedStripe(message, TFT_WHITE, leleConfig.priColor);
    }
    tft.fillRect(20, tftHeight - 45, barWidth, 13, leleConfig.priColor);
}
...
void drawSubmenu(int index, std::vector<Option> &options, const char *title) {
    ...
    tft.setTextSize(FP);
    tft.drawPixel(0, 0, 0);
    tft.fillRect(6, 30, tftWidth - 12, 8 * FP, leleConfig.bgColor);
    ...
    tft.setTextColor(leleConfig.secColor);
    tft.fillRect(6, middle_up, tftWidth - 12, 8 * FM, leleConfig.bgColor);
    ...
    tft.fillRect(6, middle - FG * LH / 2 - 1, tftWidth - 12, FG * LH + 5, leleConfig.bgColor);
    ...
    tft.setTextSize(FM);
    tft.setTextColor(leleConfig.secColor);
    tft.fillRect(6, middle_down, tftWidth - 12, 8 * FM, leleConfig.bgColor);
    tft.drawCentreString(thirdOption, tftWidth / 2, middle_down, SMOOTH_FONT);

    tft.fillRect(tftWidth - 5, 0, 5, tftHeight, leleConfig.bgColor);
    tft.fillRect(tftWidth - 5, index * tftHeight / menuSize, 5, tftHeight / menuSize, leleConfig.priColor);

#if defined(HAS_TOUCH)
    tft.drawCentreString("\\/", tftWidth / 2, middle_down + (FM * LH + 6), 1);
    tft.setTextColor(getColorVariation(leleConfig.priColor), leleConfig.bgColor);
...
void drawStatusBar() {
    ...
    if (leleConfig.theme.border) {
        tft.drawRoundRect(5, 5, tftWidth - 10, tftHeight - 10, 5, leleConfig.priColor);
        tft.drawLine(5, 25, tftWidth - 6, 25, leleConfig.priColor);
    }

    if (clock_set) {
        setTftDisplay(12, 12, leleConfig.priColor, 1, leleConfig.bgColor);
#if defined(HAS_RTC)
        _rtc.GetTime(&_time);
        ...
        setTftDisplay(12, 12, leleConfig.priColor, 1, leleConfig.bgColor);
        tft.print("Lele " + String(LELE_VERSION));
    }
}
...
void drawMainBorder(bool clear) {
    ...
    // if(wifiConnected) {tft.print(timeStr);} else {tft.print("Lele 1.0b");}
...
void printCenterFootnote(String text) {
    tft.fillRect(10, tftHeight - BORDER_PAD_X - FP * LH, tftWidth - 20, FP * LH, leleConfig.bgColor);
    tft.setTextSize(FP);
...
void drawBatteryStatus(uint8_t bat) {
    ...
    uint16_t color = leleConfig.priColor;
    uint16_t barcolor = leleConfig.priColor;
    if (bat < 16) barcolor = color = TFT_RED;
    else if (bat < 34) barcolor = color = TFT_YELLOW;
    if (charging) color = TFT_GREEN;

    tft.drawRoundRect(tftWidth - 43, 6, 36, 19, 2, charging ? color : leleConfig.bgColor); // (bolder border)
    ...
    tft.drawLine(tftWidth - 30, 9, tftWidth - 30, 9 + 13, leleConfig.bgColor);
    tft.drawLine(tftWidth - 20, 9, tftWidth - 20, 9 + 13, leleConfig.bgColor);
}
/***************************************************************************************
...
void drawWireguardStatus(int x, int y) {
    tft.fillRect(x, y, 20, 17, leleConfig.bgColor);
    if (isConnectedWireguard) {
        tft.drawRoundRect(11 + x, 0 + y, 8, 12, 5, TFT_GREEN);
        tft.fillRoundRect(10 + x, 8 + y, 10, 8, 0, TFT_GREEN);
    } else {
        tft.drawRoundRect(1 + x, 0 + y, 8, 12, 5, leleConfig.priColor);
        tft.fillRoundRect(0 + x, 8 + y, 10, 8, 0, leleConfig.bgColor);
        tft.fillRoundRect(6 + x, 8 + y, 10, 10, 0, leleConfig.priColor);
    }
}
    ...
    while (i < arraySize) {
        if (i >= start) {
            ...
            if (fileList[i].folder == true)
                tft.setTextColor(getColorVariation(leleConfig.priColor), leleConfig.bgColor);
            else if (fileList[i].operation == true) tft.setTextColor(ALCOLOR, leleConfig.bgColor);
            else { tft.setTextColor(leleConfig.priColor, leleConfig.bgColor); }
            ...
            if (index == i) {
                ...
                coord.size = nchars;
                coord.fgcolor =
                    fileList[i].folder ? getColorVariation(leleConfig.priColor) : leleConfig.priColor;
                coord.bgcolor = leleConfig.bgColor;
            } else txt = " ";
...
void drawWifiSmall(int x, int y) {
    tft.fillRect(x, y, 16, 16, leleConfig.bgColor);
    tft.fillCircle(9 + x, 14 + y, 1, leleConfig.priColor);
    tft.drawArc(9 + x, 14 + y, 4, 6, 130, 230, leleConfig.priColor, leleConfig.bgColor);
    tft.drawArc(9 + x, 14 + y, 10, 12, 130, 230, leleConfig.priColor, leleConfig.bgColor);
}

void drawWebUISmall(int x, int y) {
    tft.fillRect(x, y, 16, 16, leleConfig.bgColor);

    tft.drawCircle(8 + x, 8 + y, 7, leleConfig.priColor);

    tft.drawLine(3 + x, 4 + y, 14 + x, 4 + y, leleConfig.priColor);
    tft.drawLine(2 + x, 8 + y, 15 + x, 8 + y, leleConfig.priColor);
    tft.drawLine(3 + x, 12 + y, 14 + x, 12 + y, leleConfig.priColor);
}

void drawBLESmall(int x, int y) {
    tft.fillRect(x, 2 + y, 17, 13, leleConfig.bgColor);
    tft.drawWideLine(8 + x, 8 + y, 4 + x, 5 + y, 2, leleConfig.priColor, leleConfig.bgColor);
    tft.drawWideLine(8 + x, 8 + y, 4 + x, 13 + y, 2, leleConfig.priColor, leleConfig.bgColor);
    tft.drawTriangle(8 + x, 8 + y, 8 + x, 2 + y, 13 + x, 5 + y, leleConfig.priColor);
    tft.drawTriangle(8 + x, 8 + y, 8 + x, 14 + y, 13 + x, 11 + y, leleConfig.priColor);
}

void drawBLE_beacon(int x, int y, uint16_t color) {
    tft.fillRect(x, y, 40, 80, leleConfig.bgColor);
    tft.drawWideLine(40 + x, 53 + y, 2 + x, 26 + y, 5, color, leleConfig.bgColor);
    tft.drawWideLine(40 + x, 26 + y, 2 + x, 53 + y, 5, color, leleConfig.bgColor);
    tft.drawWideLine(40 + x, 53 + y, 20 + x, 68 + y, 5, color, leleConfig.bgColor);
    tft.drawWideLine(40 + x, 26 + y, 20 + x, 12 + y, 5, color, leleConfig.bgColor);
    tft.drawWideLine(20 + x, 12 + y, 20 + x, 68 + y, 5, color, leleConfig.bgColor);
...
void drawGPS(int x, int y) {
    tft.fillRect(x, y, 80, 80, leleConfig.bgColor);
    tft.drawEllipse(40 + x, 70 + y, 15, 8, leleConfig.priColor);
    tft.drawArc(40 + x, 25 + y, 23, 7, 0, 340, leleConfig.priColor, leleConfig.bgColor);
    tft.fillTriangle(40 + x, 70 + y, 20 + x, 64 + y, 60 + x, 64 + y, leleConfig.priColor);
}

void drawGpsSmall(int x, int y) {
    tft.fillRect(x, y, 17, 17, leleConfig.bgColor);
    tft.drawEllipse(9 + x, 14 + y, 4, 3, leleConfig.priColor);
    tft.drawArc(9 + x, 6 + y, 5, 2, 0, 340, leleConfig.priColor, leleConfig.bgColor);
    tft.fillTriangle(9 + x, 15 + y, 5 + x, 9 + y, 13 + x, 9 + y, leleConfig.priColor);
}

void drawCreditCard(int x, int y) {
    tft.fillRect(x, y, 70, 50, leleConfig.bgColor);
    tft.fillRoundRect(x + 5, y + 5, 60, 40, 5, leleConfig.priColor);
    tft.fillRect(x + 5, y + 15, 60, 10, getColorVariation(leleConfig.priColor, 3, -1));
    tft.fillRect(x + 10, y + 30, 12, 10, getColorVariation(leleConfig.priColor, 3, 1));
    tft.drawRect(x + 10, y + 30, 12, 10, getColorVariation(leleConfig.priColor, 5, -1));
    tft.drawRect(x + 10 + 4, y + 30, 4, 10, getColorVariation(leleConfig.priColor, 5, -1));
    tft.drawRect(x + 10, y + 33, 5, 4, getColorVariation(leleConfig.priColor, 5, -1));
    tft.drawRect(x + 17, y + 33, 5, 4, getColorVariation(leleConfig.priColor, 5, -1));
    tft.fillRect(x + 30, y + 35, 30, 5, getColorVariation(leleConfig.priColor, 5, 1));
}

void drawMfkey32Icon(int x, int y) {
    tft.drawRect(x + 2, y + 15, 24, 40, leleConfig.priColor);
    tft.drawRect(x + 5, y + 18, 18, 12, leleConfig.priColor);
    tft.drawRect(x + 5, y + 34, 18, 18, leleConfig.priColor);
    tft.drawLine(x + 5, y + 40, x + 22, y + 40, leleConfig.priColor);
    tft.drawLine(x + 5, y + 46, x + 22, y + 46, leleConfig.priColor);
    tft.drawLine(x + 11, y + 34, x + 11, y + 51, leleConfig.priColor);
    tft.drawLine(x + 17, y + 34, x + 17, y + 51, leleConfig.priColor);
    tft.drawRect(x + 30, y + 10, 25, 35, leleConfig.priColor);
    int startX = x + 32;
    int startY = y + 12;
    int endX = x + 52;
    int endY = y + 32;
    ...
    while (startX <= endX && startY <= endY && turns < 3) {
        for (int i = startX; i <= endX; i++) { tft.drawPixel(i, startY, leleConfig.priColor); }
        startY += step;
        for (int i = startY; i <= endY; i++) { tft.drawPixel(endX, i, leleConfig.priColor); }
        endX -= step;
        for (int i = endX; i >= startX; i--) { tft.drawPixel(i, endY, leleConfig.priColor); }
        endY -= step;
        for (int i = endY; i >= startY; i--) { tft.drawPixel(startX, i, leleConfig.priColor); }
        startX += step;
        turns++;
    }
    tft.fillRect(x + 40, y + 36, 6, 6, getColorVariation(leleConfig.priColor, 3, 1));
}

void drawMfkey64Icon(int x, int y) {
    drawMfkey32Icon(x, y);
    tft.fillRoundRect(x + 40, y + 6, 24, 14, 4, leleConfig.bgColor);
    tft.drawRoundRect(x + 40, y + 6, 24, 14, 4, getColorVariation(leleConfig.priColor, 3, -1));
    tft.drawCircle(x + 48, y + 12, 4, getColorVariation(leleConfig.priColor, 3, -1));
}
...
bool showJpeg(FS &fs, String filename, int x, int y, bool center) {
    ...
    if (data_array) {
        ...
        vTaskDelay(1000 / portTICK_PERIOD_MS);  // Otimizado: era delay(2500)
...
int PNGDraw(PNGDRAW *pDraw) {
    ...
    uint8_t r = ((uint16_t)leleConfig.bgColor & 0xF800) >> 8;
    uint8_t g = ((uint16_t)leleConfig.bgColor & 0x07E0) >> 3;
    uint8_t b = ((uint16_t)leleConfig.bgColor & 0x001F) << 3;
