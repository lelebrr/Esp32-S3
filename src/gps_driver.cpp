...
double GPSDriver::getSpeedKmh() {
    return _gps.speed.kmph();
}

double GPSDriver::getCourse() {
...
bool GPSDriver::logPosition(const char* filename, 
                            const char* ssid,
                            const char* bssid,
                            int rssi) {
...
bool GPSDriver::exportToKML(const char* inputFile, const char* outputFile) {
    ...
    kml += "<Document>\n";
    kml += "  <name>Monster S3 Wardriving Log</name>\n";
    ...
    if (result) {
        Serial.printf("[GPS] Exported %d points to KML\n", placemarkCount);
    }
    
    return result;
}

TinyGPSPlus& GPSDriver::getGPS() {
    return _gps;
}
