...
bool ARPSpoofer::arpPCAPfile() {
    ...
    while (fs->exists(String("/LelePCAP/ARP_session_" + String(nf++) + ".pcap").c_str())) yield();
    pcapFile = fs->open(String("/LelePCAP/ARP_session_" + String(nf) + ".pcap").c_str(), FILE_WRITE);
...
void ARPSpoofer::setup(const Host &host, IPAddress gateway) {
    ...
    if (mitm) {
        ...
        // padprintln("/LelePCAP/ARP_session_" + String(nf) + ".pcap");
