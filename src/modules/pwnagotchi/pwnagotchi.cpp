...
void Lelegotchi_setup() {
    initPwngrid();
    initUi();
    state = STATE_INIT;
    Serial.println("Lelegotchi Initialized");
}

void Lelegotchi_update() {
...
void Lelegotchi_start() {
    ...
    if (setupSdCard()) {
        isLittleFS = false;
        if (!SD.exists("/LelePCAP")) SD.mkdir("/LelePCAP");
        if (!SD.exists("/LelePCAP/handshakes")) SD.mkdir("/LelePCAP/handshakes");
        handshakeFs = &SD;
    } else {
        if (!LittleFS.exists("/LelePCAP")) LittleFS.mkdir("/LelePCAP");
        if (!LittleFS.exists("/LelePCAP/handshakes")) LittleFS.mkdir("/LelePCAP/handshakes");
        isLittleFS = true;
    ...
    Lelegotchi_setup(); // Starts the thing
    // Draw footer & header
    drawTopCanvas();
    drawBottomCanvas();
