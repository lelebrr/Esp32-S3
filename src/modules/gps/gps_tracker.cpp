...
void GPSTracker::add_initial_file_data(File file) {
    ...
    file.println("  creator=\"Lele Firmware\"");
    ...
    file.println("    <name>Lele GPS Tracker</name>");
    file.println("    <desc>GPS Tracker using Lele Firmware</desc>");
    file.println("    <link href=\"https://Lele.computer\">");
    file.println("      <text>Lele Website</text>");
    file.println("    </link>");
    file.println("  </metadata>");
    file.println("  <trk>");
    file.println("    <name>Lele Route</name>");
    file.println("    <desc>GPS route captured by Lele Firmware</desc>");
    file.println("    <trkseg>");
}
...
void GPSTracker::add_final_file_data() {
    ...
    if (filename == "" || !(*fs).exists("/LeleGPS/" + filename)) return;

    File file = (*fs).open("/LeleGPS/" + filename, FILE_APPEND);
...
void GPSTracker::add_coord() {
    ...
    if (!(*fs).exists("/LeleGPS")) (*fs).mkdir("/LeleGPS");

    bool is_new_file = false;
    if (!(*fs).exists("/LeleGPS/" + filename)) is_new_file = true;
    File file = (*fs).open("/LeleGPS/" + filename, is_new_file ? FILE_WRITE : FILE_APPEND);
...
void GPSTracker::releasePins() {
    ...
    if (leleConfigPins.gps_bus.rx == sharedControlPin) {
