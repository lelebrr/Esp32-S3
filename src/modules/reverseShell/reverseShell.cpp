...
void ReverseShell() {
    ...
    if (!WiFi.softAP("LeleShell", "", 1)) {
        tft.println("Failed to start AP");
        return;
    }

    tft.println("Wi-Fi AP Started: LeleShell");
    ...
    webServer.on("/", [&webServer]() {
        String html = R"rawliteral(
            ...
            <html>
            <head>
                <title>LeleShell Web Interface</title>
                <style>
    ...
    while (true) {
        ...
        if (!shellConnected) {
            ...
            if (tcpClient) {
                ...
                tcpClient.println("~Welcome to LeleShell.");
