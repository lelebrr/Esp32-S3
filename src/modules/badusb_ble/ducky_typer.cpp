...
const DuckyCommand duckyCmds[]{
    ...
    {"Excluir",         KEY_DELETE,       DuckyCommandType_Cmd        },
...
void ducky_startKb(HIDInterface *&hid, bool ble) {
    ...
    if (hid == nullptr) {
        ...
        if (ble) {
            ...
            hid = new BleKeyboard(leleConfig.bleName, "LeleFW", 100);
        } else {
#if defined(USB_as_HID)
    ...
    if (ble) {
        if (hid->isConnected()) {
            ...
            hid->setLayout(keyboardLayouts[leleConfig.badUSBBLEKeyboardLayout]);
            hid->setDelay(leleConfig.badUSBBLEKeyDelay);
            return;
        }
        if (!_Ask_for_restart) _Ask_for_restart = 1; // arm the flag
        hid->begin(keyboardLayouts[leleConfig.badUSBBLEKeyboardLayout]);
        hid->setDelay(leleConfig.badUSBBLEKeyDelay);
    } else {
#if defined(USB_as_HID)
        hid->begin(keyboardLayouts[leleConfig.badUSBBLEKeyboardLayout]);
        ...
        mySerial.begin(CH9329_DEFAULT_BAUDRATE, SERIAL_8N1, BAD_RX, BAD_TX);
        delay(100);
        hid->begin(mySerial, keyboardLayouts[leleConfig.badUSBBLEKeyboardLayout]);
        hid->setDelay(leleConfig.badUSBBLEKeyDelay);
#endif
    }
}
