...
void IRAM_ATTR onPulse() {
    ...
    if (digitalRead(leleConfigPins.CC1101_bus.io0)) {
...
void rf_listen() {
    ...
    while (!check(SelPress) && !check(EscPress)) {
        ...
        if (redraw) {
            ...
            displayRedStripe(text, getComplementaryColor2(leleConfig.priColor), leleConfig.priColor);
        }
    ...
    attachInterrupt(digitalPinToInterrupt(leleConfigPins.CC1101_bus.io0), onPulse, CHANGE);
    displayRedStripe("Listening...", getComplementaryColor2(leleConfig.priColor), leleConfig.priColor);
    ...
    while (!check(EscPress)) {
        displayRedStripe(
            "Waiting for a pulse", getComplementaryColor2(leleConfig.priColor), leleConfig.priColor
        );
        if (newPulse) {
            newPulse = false;
            ...
            displayRedStripe(pulseText, getComplementaryColor2(leleConfig.priColor), leleConfig.priColor);
#if defined(BUZZ_PIN)
        ...
        if (pulseActive && millis() - lastPulseTime > 3000) {
            pulseActive = false;
            displayRedStripe("No signal", getComplementaryColor2(leleConfig.priColor), leleConfig.priColor);
        }
    ...
    detachInterrupt(digitalPinToInterrupt(leleConfigPins.CC1101_bus.io0));
}
