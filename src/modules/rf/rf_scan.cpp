...
void RFScan::setup() {
    if (!initRfModule("rx", leleConfig.rfFreq)) { return; }

    RCSwitch_Enable_Receive(rcswitch);

    if (leleConfig.rfScanRange < 0 || leleConfig.rfScanRange > 3) { leleConfig.setRfScanRange(3); }
    if (leleConfig.rfModule != CC1101_SPI_MODULE) { leleConfig.setRfFxdFreq(1); }

    display_info(received, signals, ReadRAW, codesOnly, autoSave, title);

    if (leleConfig.rfFxdFreq) frequency = leleConfig.rfFreq;
...
void RFScan::RCSwitch_Enable_Receive(RCSwitch rcswitch) {
    if (leleConfig.rfModule == CC1101_SPI_MODULE) {
        rcswitch.enableReceive(leleConfigPins.CC1101_bus.io0);
    } else {
        rcswitch.enableReceive(leleConfig.rfRx);
    }
}
...
bool RFScan::fast_scan() {
    ...
    if (idx < range_limits[leleConfig.rfScanRange][0] || idx > range_limits[leleConfig.rfScanRange][1]) {
        idx = range_limits[leleConfig.rfScanRange][0];
    }
    ...
    if (rssi > rssiThreshold) {
        ...
        if (_try >= _MAX_TRIES) {
            ...
            leleConfig.setRfFreq(_freqs[max_index].freq, 2); // change to fixed frequency
...
void RFScan::select_menu_option() {
    ...
    if (leleConfig.rfModule == CC1101_SPI_MODULE && !leleConfig.rfFxdFreq)
...
void RFScan::set_range() {
    ...
    options = {
        {String("Fxd [" + String(leleConfig.rfFreq) + "]").c_str(),
         [=]() { leleConfig.setRfScanRange(leleConfig.rfScanRange, 1); }                                   },
        {"Choose Fxd",                                               [&]() { chooseFixedOpt = true; }        },
        {subghz_frequency_ranges[0],                                 [=]() { leleConfig.setRfScanRange(0); }},
        {subghz_frequency_ranges[1],                                 [=]() { leleConfig.setRfScanRange(1); }},
        {subghz_frequency_ranges[2],                                 [=]() { leleConfig.setRfScanRange(2); }},
        {subghz_frequency_ranges[3],                                 [=]() { leleConfig.setRfScanRange(3); }},
    };
    ...
    if (chooseFixedOpt) { // Range
        ...
        for (int i = 0; i < arraySize; i++) {
            ...
            options.emplace_back(tmp.c_str(), [=]() { leleConfig.rfFreq = subghz_frequency_list[i]; });
    ...
    if (leleConfig.rfFxdFreq) displayTextLine("Scan freq set to " + String(leleConfig.rfFreq));
    else displayTextLine("Range set to " + String(subghz_frequency_ranges[leleConfig.rfScanRange]));
}
...
void display_info(RfCodes received, int signals, bool ReadRAW, bool codesOnly, bool autoSave, String title) {
    ...
    if (leleConfig.rfFxdFreq) padprintln("Scanning: " + String(leleConfig.rfFreq) + " MHz");
    else padprintln("Scanning: " + String(subghz_frequency_ranges[leleConfig.rfScanRange]));
...
bool RCSwitch_SaveSignal(float frequency, RfCodes codes, bool raw, char *key, bool autoSave) {
    ...
    String subfile_out = "Filetype: Lele SubGhz File\nVersion 1\n";
...
String RCSwitch_Read(float frequency, int max_loops, bool raw) {
    ...
    if (!frequency) frequency = leleConfig.rfFreq; // default from config
