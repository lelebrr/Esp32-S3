...
void sendCustomRF() {
    ...
    while (1) {
        delay(200);
        filepath = loopSD(*fs, true, "SUB", "/LeleRF");
...
void sendRfCommand(struct RfCodes rfcode, bool hideDefaultUI) {
    ...
    */
    ...
    // digitalWrite(leleConfig.rfTx, LED_OFF);
    deinitRfModule();
}
...
void RCSwitch_send(uint64_t data, unsigned int bits, int pulse, int protocol, int repeat) {
    ...
    if (leleConfig.rfModule == CC1101_SPI_MODULE) {
        mySwitch.enableTransmit(leleConfigPins.CC1101_bus.io0);
    } else {
        mySwitch.enableTransmit(leleConfig.rfTx);
    }
...
void RCSwitch_RAW_Bit_send(RfCodes data) {
    int nTransmitterPin = leleConfig.rfTx;
    if (leleConfig.rfModule == CC1101_SPI_MODULE) { nTransmitterPin = leleConfigPins.CC1101_bus.io0; }
