...
void sinewave_animation() {
    ...
    for (int x = 20; x < tftWidth - 20; x++) {
        ...
        tft.drawFastVLine(x, lastY, sinewaveWidth, leleConfig.bgColor);
        tft.drawFastVLine(x, y, sinewaveWidth, leleConfig.priColor);
    }
...
void rf_raw_record_draw(RawRecordingStatus status) {
    ...
    if (status.frequency <= 0) {
        ...
        // Draw the bar
        tft.drawFastVLine(x, yTop, barHeight * 2, leleConfig.priColor);
    }
}
...
float rf_freq_scan() {
    float frequency = 0;
    int idx = range_limits[leleConfig.rfScanRange][0];
    uint8_t attempt = 0;
    ...
    while (frequency <= 0 && !check(EscPress)) { // FastScan
        ...
        if (leleConfig.rfModule == CC1101_SPI_MODULE) {
            if (idx < range_limits[leleConfig.rfScanRange][0] ||
                idx > range_limits[leleConfig.rfScanRange][1]) {
            ...
            if (rssi > rssiThreshold) {
                ...
                if (attempt >= FREQUENCY_SCAN_MAX_TRIES) {
                    ...
                    leleConfig.setRfFreq(best_frequencies[max_index].freq, 0);
...
void rf_range_selection(float currentFrequency = 0.0) {
    int option = 0;
    options = {
        {String("Fixed [" + String(leleConfig.rfFreq) + "]").c_str(),
         [=]() { leleConfig.setRfFreq(leleConfig.rfFreq, 2); }                                               },
    ...
    if (option == 1) { // Fixed Frequency Selector
        ...
        for (int i = 0; i < arraySize; i++) {
            ...
            options.push_back({tmp.c_str(), [=]() { leleConfig.setRfFreq(subghz_frequency_list[i], 2); }});
...
void rf_raw_record_create(RawRecording &recorded, bool &returnToMenu) {
    ...
    rssiFeature = leleConfig.rfModule == CC1101_SPI_MODULE;

    tft.fillScreen(leleConfig.bgColor);
    drawMainBorder();

    if (rssiFeature) rf_range_selection(leleConfig.rfFreq);
    ...
    if (leleConfig.rfModule == CC1101_SPI_MODULE) {
        if (leleConfig.rfFxdFreq || !rssiFeature) status.frequency = leleConfig.rfFreq;
        else status.frequency = rf_freq_scan();
    } else status.frequency = leleConfig.rfFreq;
    ...
    tft.drawPixel(0, 0, 0);
    tft.fillRect(10, 30, tftWidth - 20, tftHeight - 40, leleConfig.bgColor);
    rf_raw_record_draw(status);
