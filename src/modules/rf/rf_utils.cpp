...
// NVS para persistência de códigos RF recentes
static Preferences rfPrefs;
static const char* RF_NVS_NAMESPACE = "rf_recent";
...
// Funções para salvar/carregar códigos RF recentes em NVS
void saveRecentRfCodes() {
    rfPrefs.begin(RF_NVS_NAMESPACE, false);
    rfPrefs.putInt("last_used", recent_rfcodes_last_used);
    for (int i = 0; i < 16; i++) {
        String prefix = "rf" + String(i) + "_";
        rfPrefs.putString((prefix + "path").c_str(), recent_rfcodes[i].filepath);
        rfPrefs.putFloat((prefix + "freq").c_str(), recent_rfcodes[i].frequency);
        rfPrefs.putString((prefix + "proto").c_str(), recent_rfcodes[i].protocol);
        rfPrefs.putString((prefix + "preset").c_str(), recent_rfcodes[i].preset);
    }
    rfPrefs.end();
    Serial.println("[RF] Códigos recentes salvos em NVS");
}

void loadRecentRfCodes() {
    rfPrefs.begin(RF_NVS_NAMESPACE, true);
    if (rfPrefs.isKey("last_used")) {
        recent_rfcodes_last_used = rfPrefs.getInt("last_used", 0);
        for (int i = 0; i < 16; i++) {
            String prefix = "rf" + String(i) + "_";
            recent_rfcodes[i].filepath = rfPrefs.getString((prefix + "path").c_str(), "");
            recent_rfcodes[i].frequency = rfPrefs.getFloat((prefix + "freq").c_str(), 433.92);
            recent_rfcodes[i].protocol = rfPrefs.getString((prefix + "proto").c_str(), "");
            recent_rfcodes[i].preset = rfPrefs.getString((prefix + "preset").c_str(), "");
        }
        Serial.println("[RF] Códigos recentes carregados de NVS");
    }
    rfPrefs.end();
}
...
bool initRfModule(String mode, float frequency) {
    ...
    if (!frequency) frequency = leleConfig.rfFreq;

    if (leleConfig.rfModule == CC1101_SPI_MODULE) { // CC1101 in use
        if (leleConfigPins.CC1101_bus.mosi == (gpio_num_t)TFT_MOSI &&
            leleConfigPins.CC1101_bus.mosi != GPIO_NUM_NC) { // (T_EMBED), CORE2 and others
#if TFT_MOSI > 0
            initCC1101once(&tft.getSPIinstance());
#else
            yield();
#endif
        } else if (leleConfigPins.CC1101_bus.mosi ==
                   leleConfigPins.SDCARD_bus.mosi) { // (CARDPUTER) and (ESP32S3DEVKITC1) and devices that
        ...
        if (mode == "tx") {
            ...
            pinMode(leleConfigPins.CC1101_bus.io0, OUTPUT);
            ...
            pinMode(leleConfigPins.CC1101_bus.io0, INPUT);
        ...
        if (abs(frequency - leleConfig.rfFreq) > 1) {
        ...
        if (mode == "tx") {
            gsetRfTxPin(false);
            if (leleConfigPins.SDCARD_bus.checkConflict(leleConfig.rfTx)) sdcardSPI.end();
            gpio_reset_pin((gpio_num_t)leleConfig.rfTx);
            pinMode(leleConfig.rfTx, OUTPUT);
            digitalWrite(leleConfig.rfTx, LOW);

        } else if (mode == "rx") {
            // Rx Mode
            gsetRfRxPin(false);
            if (leleConfigPins.SDCARD_bus.checkConflict(leleConfig.rfRx)) sdcardSPI.end();
            gpio_reset_pin((gpio_num_t)leleConfig.rfRx);
            pinMode(leleConfig.rfRx, INPUT);
        }
    }
    // no error
    return true;
...
void deinitRfModule() {
    if (leleConfig.rfModule == CC1101_SPI_MODULE) {
        if (cc1101_spi_ready) {
            ELECHOUSE_cc1101.setSidle();
            cc1101_spi_ready = false;
        }
        digitalWrite(leleConfigPins.CC1101_bus.io0, LOW);
        digitalWrite(leleConfigPins.CC1101_bus.cs, HIGH);
        ioExpander.turnPinOnOff(IO_EXP_CC_RX, LOW);
        ioExpander.turnPinOnOff(IO_EXP_CC_TX, LOW);
    } else digitalWrite(leleConfig.rfTx, LED_OFF);
}
...
void initCC1101once(SPIClass *SSPI) {
    ...
    ELECHOUSE_cc1101.setSpiPin(
        ...
        leleConfigPins.CC1101_bus.miso,
        leleConfigPins.CC1101_bus.mosi,
        leleConfigPins.CC1101_bus.cs
    );
    ELECHOUSE_cc1101.setGDO0(leleConfigPins.CC1101_bus.io0); // use Gdo0 for both Tx and Rx
...
#pragma message("Should not be using initRMT()")
    ...
    rxconfig.gpio_num = gpio_num_t(leleConfig.rfRx);

    if (leleConfig.rfModule == CC1101_SPI_MODULE)
        rxconfig.gpio_num = gpio_num_t(leleConfigPins.CC1101_bus.io0);
...
void setMHZ(float frequency) {
    ...
    if (leleConfig.rfModule == CC1101_SPI_MODULE) {
        ...
        if (leleConfigPins.CC1101_bus.cs != 17) change = false;
#endif
...
rmt_channel_handle_t setup_rf_rx() {
    if (!initRfModule("rx", leleConfig.rfFreq)) return NULL;
    setMHZ(leleConfig.rfFreq);
    rmt_rx_channel_config_t rx_channel_cfg = {};
    rx_channel_cfg.gpio_num = leleConfig.rfModule == CC1101_SPI_MODULE
                                  ? gpio_num_t(leleConfigPins.CC1101_bus.io0)
                                  : gpio_num_t(leleConfig.rfRx); // GPIO number
