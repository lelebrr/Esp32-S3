...
bool setup_rf_spectrum(RingbufHandle_t *rb) {
    if (!initRfModule("rx", leleConfig.rfFreq)) return false;
    ...
    while (1) {
...
#endif
        {
            if (rx_size != 0) {
                ...
                tft.fillRect(0, 20, tftWidth, tftHeight, leleConfig.bgColor);
                ...
                for (size_t i = 0; i < rx_size; i++) {
                    ...
                    tft.drawLine(lineX, startY, lineX, endY, leleConfig.priColor);
                }
#else
                for (size_t i = 0; i < rx_size; i++) {
                    int lineHeight = map(
...
void rf_SquareWave() {
    ...
    if (leleConfig.rfModule == CC1101_SPI_MODULE) rcswitch.enableReceive(leleConfigPins.CC1101_bus.io0);
    else rcswitch.enableReceive(leleConfig.rfRx);
    ...
    while (1) {
        if (rcswitch.RAWavailable()) {
            ...
            // Clear the display area
            // tft.fillRect(0, 0, tftWidth, tftHeight, leleConfig.bgColor);
            // Draw waveform based on signal strength
            for (int i = 0; i < RCSWITCH_RAW_MAX_CHANGES - 1; i += 2) {
                ...
                if (line_h > tftHeight) {
                    line_h = 15;
                    tft.fillRect(0, 12, tftWidth, tftHeight, leleConfig.bgColor);
                }
                tft.drawFastVLine(line_w, line_h, 6, leleConfig.priColor);
                tft.drawFastHLine(line_w, line_h, raw[i] / TIME_DIVIDER, leleConfig.priColor);

                tft.drawFastVLine(line_w + raw[i] / TIME_DIVIDER, line_h, 6, leleConfig.priColor);
                tft.drawFastHLine(
