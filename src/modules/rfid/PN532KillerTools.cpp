...
void PN532KillerTools::setup() {
    // Reset Pin states
    if (leleConfigPins.SDCARD_bus.checkConflict(RXD_PIN) ||
        leleConfigPins.SDCARD_bus.checkConflict(TXD_PIN)) {
        sdcardSPI.end();
    }
    if (leleConfigPins.CC1101_bus.checkConflict(RXD_PIN) ||
        leleConfigPins.CC1101_bus.checkConflict(TXD_PIN) ||
        leleConfigPins.NRF24_bus.checkConflict(RXD_PIN) ||
        leleConfigPins.NRF24_bus.checkConflict(TXD_PIN)) {
        CC_NRF_SPI.end();
    }
...
void PN532KillerTools::playDeviceDetectedSound() {
    ...
    if (SD.exists("/device_detected.wav")) {
        ...
        if (leleConfig.theme.boot_sound) {
            playAudioFile(
                leleConfig.themeFS(), leleConfig.getThemeItemImg(leleConfig.theme.paths.boot_sound)
            );
