...
bool Chameleon::readFileLF() {
    ...
    filepath = loopSD(*fs, true, "RFIDLF", "/LeleRFID");
...
bool Chameleon::writeFileLF(String filename) {
    ...
    if (!(*fs).exists("/LeleRFID")) (*fs).mkdir("/LeleRFID");
    if ((*fs).exists("/LeleRFID/" + filename + ".rfidlf")) {
        int i = 1;
        filename += "_";
        while ((*fs).exists("/LeleRFID/" + filename + String(i) + ".rfidlf")) i++;
        filename += String(i);
    }
    File file = (*fs).open("/LeleRFID/" + filename + ".rfidlf", FILE_WRITE);

    if (!file) { return false; }

    file.println("Filetype: Lele RFID 125kHz File");
...
bool Chameleon::writeFileHF(String filename) {
    ...
    if (!(*fs).exists("/LeleRFID")) (*fs).mkdir("/LeleRFID");
    if ((*fs).exists("/LeleRFID/" + filename + ".rfid")) {
        int i = 1;
        filename += "_";
        while ((*fs).exists("/LeleRFID/" + filename + String(i) + ".rfid")) i++;
        filename += String(i);
    }
    File file = (*fs).open("/LeleRFID/" + filename + ".rfid", FILE_WRITE);
...
void Chameleon::saveScanResult() {
    ...
    if ((*fs).exists("/LeleRFID/Scans/" + filename + ".rfidscan")) {
        int i = 1;
        filename += "_";
        while ((*fs).exists("/LeleRFID/Scans/" + filename + String(i) + ".rfidscan")) i++;
        filename += String(i);
    }
    File file = (*fs).open("/LeleRFID/Scans/" + filename + ".rfidscan", FILE_WRITE);

    if (!file) { return; }

    file.println("Filetype: Lele RFID Scan Result");
