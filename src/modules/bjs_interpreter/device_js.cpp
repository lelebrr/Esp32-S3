...
duk_ret_t putPropDeviceFunctions(duk_context *ctx, duk_idx_t obj_idx, uint8_t magic) {
    ...
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "getLeleVersion", native_getLeleVersion, 0, magic);
...
duk_ret_t registerDevice(duk_context *ctx) {
    ...
    bduk_register_c_lightfunc(ctx, "getLeleVersion", native_getLeleVersion, 0);
    bduk_register_c_lightfunc(ctx, "getBattery", native_getLeleVersion, 0);
...
duk_ret_t native_getDeviceName(duk_context *ctx) {
    const char *deviceName = leleConfig.wifiAp.ssid != NULL ? leleConfig.wifiAp.ssid.c_str() : "LELE";
...
duk_ret_t native_getLeleVersion(duk_context *ctx) {
    duk_push_string(ctx, LELE_VERSION);
    return 1;
}
