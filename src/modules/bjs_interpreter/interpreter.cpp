...
void interpreterHandler(void *pvParameters) {
    ...
    // Native stack check usando uxTaskGetStackHighWaterMark
    UBaseType_t stackHighWaterMark = uxTaskGetStackHighWaterMark(NULL);
    if (stackHighWaterMark < 512) {
        Serial.printf("Warning: Low stack space: %d bytes\n", stackHighWaterMark * 4);
    }
    ...
    // registerAudio(ctx);
    // registerBadUSB(ctx);
    // BLE UART API - Stub preparado para futuro wrapper
    // Referência: https://github.com/pr3y/Lele/pull/1133
    // registerDevice(ctx);
    // registerDialog(ctx);
    // registerDisplay(ctx);
    // registerGPIO(ctx);
    ...
    // Sintaxe Flipper - Parcialmente compatível
    // Referência: https://github.com/jamisonderek/flipper-zero-tutorials/wiki/JavaScript
    // API Duktape: https://duktape.org/api.html
...
duk_ret_t native_require(duk_context *ctx) {
    ...
    if (filepath == "audio") {
        ...
        // The more compatible we are, the more Node.js scripts can run on Lele
