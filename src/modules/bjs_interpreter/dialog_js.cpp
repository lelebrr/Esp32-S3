...
duk_ret_t putPropDialogFunctions(duk_context *ctx, duk_idx_t obj_idx, uint8_t magic) {
    ...
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "Sucesso", native_dialogNotification, 2, magic + 1);
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "Aviso", native_dialogNotification, 2, magic + 2);
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "Erro", native_dialogNotification, 2, magic + 3);
...
duk_ret_t native_dialogMessage(duk_context *ctx) {
    ...
    int8_t selectedButton =
        displayMessage(duk_to_string(ctx, 0), leftButton, centerButton, rightButton, leleConfig.priColor);
    if (selectedButton == 0)
...
duk_ret_t native_dialogChoice(duk_context *ctx) {
    ...
    if (legacy) {
        options.push_back({"Cancelar", [&]() { result = ""; }});
    }
