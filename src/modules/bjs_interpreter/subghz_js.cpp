...
duk_ret_t putPropSubGHzFunctions(duk_context *ctx, duk_idx_t obj_idx, uint8_t magic) {
    ...
    bduk_put_prop_c_lightfunc(ctx, obj_idx, "getFrequency", native_subghzGetFrequency, 0, magic);
...
duk_ret_t native_subghzRead(duk_context *ctx) {
    ...
    String r = "";
    if (duk_is_number(ctx, 0)) r = RCSwitch_Read(leleConfig.rfFreq, duk_to_int(ctx, 0)); // custom timeout
    else r = RCSwitch_Read(leleConfig.rfFreq, 10);
...
duk_ret_t native_subghzReadRaw(duk_context *ctx) {
    ...
    if (duk_is_number(ctx, 0))
        r = RCSwitch_Read(leleConfig.rfFreq, duk_to_int(ctx, 0),
                          true); // custom timeout
    else r = RCSwitch_Read(leleConfig.rfFreq, 10, true);
    duk_push_string(ctx, r.c_str());
    return 1;
}
...
duk_ret_t native_subghzSetFrequency(duk_context *ctx) {
    ...
    if (duk_is_number(ctx, 0)) leleConfig.rfFreq = duk_to_number(ctx, 0); // float global var
    return 0;
}

duk_ret_t native_subghzGetFrequency(duk_context *ctx) {
    // usage: subghzGetFrequency() -> returns current RF frequency
    duk_push_number(ctx, leleConfig.rfFreq);
    return 1;
}
