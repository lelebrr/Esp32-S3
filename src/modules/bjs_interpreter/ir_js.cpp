...
duk_ret_t putPropIRFunctions(duk_context *ctx, duk_idx_t obj_idx, uint8_t magic) {
    ...
    // Nota: transmit(string) pode ser implementado futuramente via serialCli.parse
    return 0;
}
...
duk_ret_t native_irTransmit(duk_context *ctx) {
    ...
    // Uso alternativo possível: irTransmit(protocol, address, command)
    // Implementação via serialCli.parse("ir tx " + protocol + " " + address + " " + command)
    duk_push_boolean(ctx, r);
    return 1;
}
