...
void MorphDriver::init() {
    // Note: Serial.begin() now called in main.cpp before this
    // Wait for USB CDC to reconnect after upload/reset
    delay(500);

    Serial.println("\n========================================");
    Serial.println("        MORPHNODE FIRMWARE v2.0");
    Serial.println("========================================");
    Serial.println("[MORPH] Booting specialized hardware...");
...
void MorphDriver::initPowerPins() {
...
void MorphDriver::powerOnModules(bool wifi, bool ble, bool nfc, bool cc1101) {
...
void MorphDriver::deepSleep(uint64_t time_ms) {
...
bool MorphDriver::isPsramReady() { return psramFound(); }

float MorphDriver::getBatteryVoltage() {
    // =========================================================================
    ...
    if (millis() - lastRead < 5000 && lastVoltage > 0) { return lastVoltage; }
    lastRead = millis();
...
float MorphDriver::getTemperature() {
    // Try to get temperature from RTC if available
#if DS3231_ENABLED
    if (RTCDriver::isRunning()) { return RTCDriver::getTemperature(); }
#endif
