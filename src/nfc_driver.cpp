...
bool NFCDriver::isReady() { return _initialized; }
...
bool NFCDriver::readBlock(uint8_t blockNumber, uint8_t *data, uint8_t *key) {
    ...
    if (!_nfc.mifareclassic_AuthenticateBlock(
            _lastCard.uid, _lastCard.uidLength, blockNumber, keyType, authKey
        )) {
...
bool NFCDriver::cloneToFile(const char *filename) {
    ...
    for (int block = 0; block < maxBlocks; block++) {
        ...
        if (block % 4 == 0) {
            if (!_nfc.mifareclassic_AuthenticateBlock(
                    _lastCard.uid, _lastCard.uidLength, block, 0, _defaultKey
                )) {
                // Try key B
                if (!_nfc.mifareclassic_AuthenticateBlock(
                        _lastCard.uid, _lastCard.uidLength, block, 1, _defaultKey
                    )) {
        ...
        if (_nfc.mifareclassic_ReadDataBlock(block, blockData)) {
            ...
            char hexStr[33];
            for (int i = 0; i < 16; i++) { sprintf(&hexStr[i * 2], "%02X", blockData[i]); }
            blocks.add(hexStr);
        } else {
    ...
    if (result) { Serial.printf("[NFC] Card cloned to %s\n", filename); }
...
// UTILITY FUNCTIONS
// ============================================================================
NFCCard NFCDriver::getLastCard() { return _lastCard; }
