...
void __attribute__((weak)) taskInputHandler(void *parameter) {
    ...
    while (true) {
        ...
        // IA toma decisões periodicamente
...
#define POWER_SWITCH 99            // VCC bateria → BMS (não GPIO, controle de energia)
...
// Funções para novos componentes de hardware
...
void check_fault_injection() {
    ...
    if (digitalRead(FAULT_INJECT_BUTTON) == LOW && millis() - last_fault_time > 1000) {
        ...
        log_w("[FAULT] Injeção de falha manual ativada");
        delay(100);
        fault_injection_active = false;
    }
}

    // Funções Q-table movidas para q_learn_ia.cpp

void ai_attack_decision() {
    // Usar nova implementação Q-learning otimizada
...
void think_and_speak(const char* text) {
    if (speech_pending) return;  // Já há fala pendente

    // Bufferizar texto em PSRAM para TTS assíncrono
    speech_pending = true;
