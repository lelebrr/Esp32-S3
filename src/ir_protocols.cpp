...
void IRProtocols::send(IRProtocolType protocol, uint64_t code, uint16_t bits) {
    ...
    switch (protocol) {
        case IR_PROTO_NEC: _irsend.sendNEC(code, bits ? bits : 32); break;

        case IR_PROTO_RC5: _irsend.sendRC5(code, bits ? bits : 12); break;

        case IR_PROTO_RC6: _irsend.sendRC6(code, bits ? bits : 20); break;

        case IR_PROTO_SONY:
            _irsend.sendSony(code, bits ? bits : 12, 2); // 2 repeats
            break;

        case IR_PROTO_SAMSUNG: _irsend.sendSAMSUNG(code, bits ? bits : 32); break;

        case IR_PROTO_LG: _irsend.sendLG(code, bits ? bits : 28); break;

        case IR_PROTO_PANASONIC: _irsend.sendPanasonic64(code, bits ? bits : 48); break;

        default: Serial.println("[IR] Unknown protocol!"); break;
    }
...
bool IRProtocols::available() { return _irrecv.decode(&_results); }
...
void IRProtocols::startBrute(
    IRProtocolType protocol, uint64_t startCode, uint64_t endCode, uint16_t delayMs
) {
    _bruteActive = true;
...
void IRProtocols::sendTVPower(uint8_t brand) {
    ...
    switch (brand) {
        ...
        default: Serial.println("[IR] Unknown brand"); break;
    }
}
