...
void NFCRelay::setPeer(const uint8_t* mac) {
    ...
    if (esp_now_add_peer(&peerInfo) != ESP_OK) {
        ...
        Serial.printf("[RELAY] Peer: %02X:%02X:%02X:%02X:%02X:%02X\n",
...
bool NFCRelay::discoverPeer() {
    ...
    while (!_peerSet && millis() - start < 5000) {
        delay(10);
    }
...
void NFCRelay::handleReaderMode() {
    ...
    if (card.valid) {
        ...
        if (_currentUidLen == 0 || memcmp(_currentUid, card.uid, card.uidLength) != 0) {
            ...
            Serial.printf("[RELAY] Card detected, UID sent: %s\n",
...
void NFCRelay::handleEmulatorMode() {
    ...
    if (_currentUidLen > 0) {
        ...
        for (int i = 0; i < _currentUidLen; i++) {
            Serial.printf("%02X", _currentUid[i]);
        }
        Serial.println();
    }
}
...
bool NFCRelay::isActive() {
    return _active;
}

uint32_t NFCRelay::getPacketsSent() {
    return _packetsSent;
}

uint32_t NFCRelay::getPacketsReceived() {
    return _packetsReceived;
}

uint32_t NFCRelay::getLatencyMs() {
    return _lastLatency;
}
