    ...
    while (http.connected() && (contentLength < 0 || downloaded < contentLength)) {
        ...
        if (available > 0) {
            ...
            if (_progressCallback && (downloaded - lastProgress > 10240)) {
                ...
                if (!_progressCallback(downloaded, contentLength, filename)) {
                    _cancelled = true;
                }
            }
        }
...
DownloadResult download_tts_voices() {
    ...
    if (!SD.exists(SD_PATH_TTS_VOICES)) {
        SD.mkdir(SD_PATH_TTS_VOICES);
    }
...
DownloadResult download_wordlists_br() {
    ...
    if (!SD.exists(SD_PATH_CVE)) {
        SD.mkdir(SD_PATH_CVE);
    }
...
DownloadResult download_all_resources() {
    ...
    Serial.println("[DL] ======================================");
    Serial.printf("[DL] COMPLETE: %d files, %d bytes, %d errors\n",
...
bool resources_check_complete() {
    ...
    Serial.printf("[DL] Resources check: voices=%d, wordlist=%d, qtable=%d\n",
